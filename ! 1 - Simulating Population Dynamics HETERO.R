...
setwd("C:/Users/Micheletti/Dropbox/PhD/Thesis/!! WHOLE DISSERTATION !!/FINAL ANALYSIS/Data/HETERO")
...
anal <- 6 # Variation = 10% < H < 60%

# Total Number of iterations
iter <- it * anal

# To save the outputs
output.Real.H <- matrix(NA,ncol=iter,nrow=5)
rownames(output.Real.H) <- c("N","B","phin","phib","p")

# FOR THE Pr LIST
MP <- VAR <- MINP <- MAXP <- numeric(iter)
Var.tb <- matrix(NA, nrow=4, ncol=iter)
rownames(Var.tb) <- c("Mean P", "Variation in CH", "Min P", "Max P")
...
for(ite in 1:iter)
{
  if (ite <= it) {variation = 0.1}
  if (ite > it && ite <= 2*it) {variation = 0.2}
  if (ite  > 2*it && ite <= 3*it) {variation = 0.3}
  if (ite  > 3*it && ite <= 4*it) {variation = 0.4}
  if (ite  > 4*it && ite <= 5*it) {variation = 0.5}
  if (ite  > 5*it && ite <= 6*it) {variation = 0.6}
  ...
  ls.H.p1.CH[[ite]] <- rbind(p.1st.f, p.1st.m)
  ...
  ##############################
  # For IPM with Heterogeneity #
  ##############################
  
  # Matrix of individual probability of recapture
  indv.rp.f <- matrix(NA, nrow=nrow(p.2nd.f), ncol=tot.occ, byrow=T)
  for (i in 1:nrow(p.2nd.f)){
    indv.rp.f[i,] <- rep(runif(1, pr-pr.var,pr+pr.var),tot.occ)}
  indv.rp.m <- matrix(NA, nrow=nrow(p.2nd.m), ncol=tot.occ, byrow=T)
  for (i in 1:nrow(p.2nd.m)){
    indv.rp.m[i,] <- rep(runif(1,pr-pr.var,pr+pr.var),tot.occ)}
  
  pr.f <- indv.rp.f*p.2nd.f
  pr.m <- indv.rp.m*p.2nd.m
  
  # Save the outputs of individual probability of resight
  PR.H <- rbind(indv.rp.f, indv.rp.m)
  ls.PR[[ite]] <- PR.H[,1]
  
  # Bernoulli trial for capture by observation
  for (i in 1:nrow(pr.f)){
    pr.f[i,] <- rbinom(tot.occ, 1, pr.f[i,])}
  for (i in 1:nrow(pr.m)){
    pr.m[i,] <- rbinom(tot.occ, 1, pr.m[i,])}
  ...
  for (i in 1:tot.occ){
    ...
    for (j in 1:nrow(cnt[[i]])){
      cnt[[i]][j,1] <- rbinom(1, 1, PR.H[j])} # Bernoulli trial: was the individual observed in that occasion?
    COUNT[i] <- sum(cnt[[i]])
  } #i
  ...
  for (i in 1:tot.occ){
    ...
    for (j in 1:nrow(cnt.d[[i]])){
      cnt.d[[i]][j,1] <- rbinom(1, 1, PR.H[j])} # Bernoulli trial: was the individual observed in that occasion?
  ...
  ls.H.CH[[ite]]  <- assign(paste("CH", ite, sep = "_"), CapHist)
  ls.H.CD[[ite]]  <- assign(paste("CD", ite, sep = "_"), Count.d)
  ls.H.CI[[ite]] <-  assign(paste("CI", ite, sep = "_"), Count.i)
  
  # Population values
  output.Real.H[1,ite]<- round(mean(N))     # Number of Individuals
  output.Real.H[2,ite]<- round(sum(B[2:tot.occ]/(n.occ-1)))     # Number of recruits
  output.Real.H[3,ite]<- phi.days    # Survival probability secondary sample
  output.Real.H[4,ite]<- phi.occ     # Survival probability primary sample
  output.Real.H[5,ite]<- mean(pr)    # Recapture probability
  
  Var.tb[1,ite] <- mean(ls.PR[[ite]])
  Var.tb[3,ite] <- min(ls.PR[[ite]])
  Var.tb[4,ite] <- max(ls.PR[[ite]])
  Var.tb[2,ite] <- max(((Var.tb[4,ite]-Var.tb[1,ite]))/Var.tb[1,ite], Mod((Var.tb[3,ite]-Var.tb[1,ite])/Var.tb[1,ite]))
  
} # End iterations

save(ls.H.CH, file="CH.RData")
save(ls.H.CD, file="CD.RData")
save(ls.H.CI, file="CI.RData")
save(ls.H.p1.CH, file="p1-CH.RData")
save(ls.H.Nf, file="N-fem.RData")
save(ls.H.Nm, file="N-male.RData")
save(ls.H.N, file="N-each.RData")
save(ls.H.S, file="S.RData")
save(ls.H.Bf, file="B-fem.RData")
save(ls.H.Bm, file="B-male.RData")
save(output.Real.H,file="Real.RData")
save(ls.PR,file="listPr.Rdata")
save(Var.tb,file="Variation.Rdata")
