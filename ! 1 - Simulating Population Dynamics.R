...
setwd("C:/Users/Micheletti/Dropbox/PhD/Thesis/!! WHOLE DISSERTATION !!/FINAL ANALYSIS/Data/1P 2P")
...
for(ite in 1:iter)
{
  ...
  for   (t in 1:tot.occ) {
    ...
    if (t %in% seq(1:(tot.occ-1))) {
      PHI.fem <- matrix(rep(phi[t], fem.all[t]), ncol = 1, nrow = (fem.all[t]), byrow = TRUE) #Survival probability Matrix for all females (adults and cubs)
      PHI.male <- matrix(rep(phi[t], male.all[t]), ncol = 1, nrow = (male.all[t]), byrow = TRUE) #Survival probability Matrix for all males (adults, juv and cubs)
  ...
  S <- sum(tot.entries, N.ini, na.rm=FALSE)
  S.f <- sum(B.f) 
  S.m <- sum(B.m)
  ...
  for (t in 1:(tot.occ-1)){
    phi.matrix[[t]] <- rbind(phi.matrix[[t]], matrix(rep(NA, abs(S.f-nrow(phi.matrix[[t]]))), ncol=1, byrow=TRUE))}
  phi.matrix.f <- matrix(unlist(phi.matrix), ncol=(tot.occ-1), byrow=FALSE)
  ...
  surv.f <- cbind(matrix(c(rep(1, fem.all[1]), rep(NA, nrow(surv.f)-fem.all[1])), ncol=1, byrow=FALSE), surv.f)
  ...
  for (t in 1:l.btw.occ){
    ifelse({fem.cubs[btw.occ[t]]==0},
           ...
           surv.f[(val.na[t]):(val.na[t]+fem.cubs[btw.occ[t]]-1),btw.occ[t]] <- matrix(rep(1,fem.cubs[btw.occ[t]]), ncol=1, byrow=FALSE)})}
  ...
  for (t in 1:(tot.occ-1)){
    phi.matrix[t] <- list(get(phi.matrix[[t]]))}
  for (t in 1:(tot.occ-1)){
    phi.matrix[[t]] <- rbind(phi.matrix[[t]], matrix(rep(NA, abs(S.m-nrow(phi.matrix[[t]]))), ncol=1, byrow=TRUE))}
  phi.matrix.m <- matrix(unlist(phi.matrix), ncol=(tot.occ-1), byrow=FALSE)
  ...
  surv.m <- cbind(matrix(c(rep(1, male.all[1]), rep(NA, nrow(surv.m)-male.all[1])), ncol=1, byrow=FALSE), surv.m)
  ...
  for (t in 1:l.btw.occ){
    ifelse({male.cubs[btw.occ[t]]==0},
           ...
           surv.m[(val.na[t]):(val.na[t]+male.cubs[btw.occ[t]]-1),btw.occ[t]] <- matrix(rep(1,male.cubs[btw.occ[t]]), ncol=1, byrow=FALSE)})}
  ...
  p.2nd.f <- matrix(0, nrow=length(p.1st.f.min), ncol=tot.occ, byrow=TRUE)
  p.2nd.m <- matrix(0, nrow=length(p.1st.m.min), ncol=tot.occ, byrow=TRUE)
  ...
  output.Real[2,ite]<- round(sum(B[2:tot.occ]/(n.occ-1)))     # Number of recruits
