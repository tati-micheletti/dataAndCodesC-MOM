...
output.1H.CMOM <- matrix(NA,ncol=iter,nrow=20)
rownames(output.1H.CMOM) <- c("CMOMd.N","CMOMd.N.sd","CMOMd.B","CMOMd.B.sd","CMOMd.phin","CMOMd.phin.sd",
...
for(ite in 1:iter)
{
  ...
  assign(paste("CMOMd.1H", ite, sep = "_"), CMOMd)
  assign(paste("CMOMi.1H", ite, sep = "_"), CMOMi)
  ...
  output.1H.CMR <- matrix(NA,ncol=iter,nrow=10)
  rownames(output.1H.CMR) <- c("CMR.N","CMR.N.sd","CMR.B","CMR.B.sd","CMR.phin","CMR.phin.sd","CMR.phib",
  ...
  # Store each output from MCMC
  assign(paste("CMR.1H", ite, sep = "_"), CMR)
  ...
  if(ite==iter){
    ...
    library(gtools)
    ls.CMOMd.1H <- mixedsort(BugsObj[grep("CMOMd.1H_", BugsObj)])
    ls.CMOMi.1H <- mixedsort(BugsObj[grep("CMOMi.1H_", BugsObj)])
    ls.CMR.1H <- mixedsort(BugsObj[grep("CMR.1H_", BugsObj)])
    
    for (j in 1:iter){
      output.1H.CMOM[1,j] <- mean(get(ls.CMOMd.1H[[j]])[[10]][11:19,"mean"])
      output.1H.CMOM[2,j] <- mean(get(ls.CMOMd.1H[[j]])[[10]][11:19,"sd"])
      output.1H.CMOM[3,j] <- mean(get(ls.CMOMd.1H[[j]])[[10]][20:21,"mean"])
      output.1H.CMOM[4,j] <- mean(get(ls.CMOMd.1H[[j]])[[10]][20:21,"sd"])
      output.1H.CMOM[5,j] <- mean(c(get(ls.CMOMd.1H[[j]])[[10]][1:2,"mean"],get(ls.CMOMd.1H[[j]])[[10]][4:5,"mean"],get(ls.CMOMd.1H[[j]])[[10]][7:8,"mean"])) 
      output.1H.CMOM[6,j] <- mean(c(get(ls.CMOMd.1H[[j]])[[10]][1:2,"sd"],get(ls.CMOMd.1H[[j]])[[10]][4:5,"sd"],get(ls.CMOMd.1H[[j]])[[10]][7:8,"sd"]))
      output.1H.CMOM[7,j] <- mean(c(get(ls.CMOMd.1H[[j]])[[10]][3,"mean"],get(ls.CMOMd.1H[[j]])[[10]][6,"mean"])) 
      output.1H.CMOM[8,j] <- mean(c(get(ls.CMOMd.1H[[j]])[[10]][3,"sd"],get(ls.CMOMd.1H[[j]])[[10]][6,"sd"]))
      output.1H.CMOM[9,j] <- get(ls.CMOMd.1H[[j]])[[10]][9,"mean"]
      output.1H.CMOM[10,j] <- get(ls.CMOMd.1H[[j]])[[10]][9,"sd"]
      
      output.1H.CMOM[11,j] <- mean(get(ls.CMOMi.1H[[j]])[[10]][11:19,"mean"])
      output.1H.CMOM[12,j] <- mean(get(ls.CMOMi.1H[[j]])[[10]][11:19,"sd"])
      output.1H.CMOM[13,j] <- mean(get(ls.CMOMi.1H[[j]])[[10]][20:21,"mean"])
      output.1H.CMOM[14,j] <- mean(get(ls.CMOMi.1H[[j]])[[10]][20:21,"sd"])
      output.1H.CMOM[15,j] <- mean(c(get(ls.CMOMi.1H[[j]])[[10]][1:2,"mean"],get(ls.CMOMi.1H[[j]])[[10]][4:5,"mean"],get(ls.CMOMi.1H[[j]])[[10]][7:8,"mean"])) 
      output.1H.CMOM[16,j] <- mean(c(get(ls.CMOMi.1H[[j]])[[10]][1:2,"sd"],get(ls.CMOMi.1H[[j]])[[10]][4:5,"sd"],get(ls.CMOMi.1H[[j]])[[10]][7:8,"sd"]))
      output.1H.CMOM[17,j] <- mean(c(get(ls.CMOMi.1H[[j]])[[10]][3,"mean"],get(ls.CMOMi.1H[[j]])[[10]][6,"mean"]))
      output.1H.CMOM[18,j] <- mean(c(get(ls.CMOMi.1H[[j]])[[10]][3,"sd"],get(ls.CMOMi.1H[[j]])[[10]][6,"sd"]))
      output.1H.CMOM[19,j] <- get(ls.CMOMi.1H[[j]])[[10]][9,"mean"]
      output.1H.CMOM[20,j] <- get(ls.CMOMi.1H[[j]])[[10]][9,"sd"]
      
      output.1H.CMR[1,j] <- mean(get(ls.CMR.1H[[j]])[[10]][11:19,"mean"])
      output.1H.CMR[2,j] <- mean(get(ls.CMR.1H[[j]])[[10]][11:19,"sd"])
      output.1H.CMR[3,j] <- mean(get(ls.CMR.1H[[j]])[[10]][20:21,"mean"])
      output.1H.CMR[4,j] <- mean(get(ls.CMR.1H[[j]])[[10]][20:21,"sd"])
      output.1H.CMR[5,j] <- mean(c(get(ls.CMR.1H[[j]])[[10]][1:2,"mean"],get(ls.CMR.1H[[j]])[[10]][4:5,"mean"],get(ls.CMR.1H[[j]])[[10]][7:8,"mean"])) 
      output.1H.CMR[6,j] <- mean(c(get(ls.CMR.1H[[j]])[[10]][1:2,"sd"],get(ls.CMR.1H[[j]])[[10]][4:5,"sd"],get(ls.CMR.1H[[j]])[[10]][7:8,"sd"]))
      output.1H.CMR[7,j] <- mean(c(get(ls.CMR.1H[[j]])[[10]][3,"mean"],get(ls.CMR.1H[[j]])[[10]][6,"mean"])) 
      output.1H.CMR[8,j] <- mean(c(get(ls.CMR.1H[[j]])[[10]][3,"sd"],get(ls.CMR.1H[[j]])[[10]][6,"sd"]))
      output.1H.CMR[9,j] <- get(ls.CMR.1H[[j]])[[10]][9,"mean"]
      output.1H.CMR[10,j] <- get(ls.CMR.1H[[j]])[[10]][9,"sd"]}
    
    save(output.1H.CMOM,file="output-1H.RData")
    save(output.1H.CMR,file="output-1H-CMR.RData")
  }}

save.image(file="1H_Backup.RData")
