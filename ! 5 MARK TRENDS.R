########################################
#                                      #
# 5b. Run Mark with simulated datasets #
#                                      #
#         Z P N E c                    #
#                                      #
########################################
...
CH.list <- CH2.list <- Mark.list.Tc <- Mark2.list.Tc <- km.list <- y.list <- km2.list <- y2.list <- Mark.list.summary.Tc <- Mark2.list.summary.Tc <- TN.ZPNEc <- TN.ZPNEs <- TNs.ZPNEc <- TNs.ZPNEs <- list()
T.output.ZPNEc <- matrix(NA,ncol=iter,nrow=10)
rownames(T.output.ZPNEc) <- c("ZPNEc.N","ZPNEc.N.se",
                            "ZPNEc.B","ZPNEc.B.se",
                            "ZPNEc.phin","ZPNEc.phin.se",
                            "ZPNEc.phib","ZPNEc.phib.se",
                            "ZPNEc.p","ZPNEc.p.se")

T.output.ZPNEs <- matrix(NA,ncol=iter,nrow=10)
rownames(T.output.ZPNEs) <- c("ZPNEs.N","ZPNEs.N.se",
                            "ZPNEs.B","ZPNEs.B.se",
...
for(ite in 1:iter){
  ...
  CH <- data.frame()
  for (i in 1:nrow(ls.T.CH[[ite]])) {
    
    # FOR THE FIRST FIELD TRIP
    CH[i,1] <- ifelse( 
      sum(ls.T.CH[[ite]][i, c(1:3)])>0,
      paste(0, sum(ls.T.CH[[ite]][i, c(1:3)]), sep = ""),
      "..")
    
    # FOR THE SECOND FIELD TRIP
    CH[i,2] <- ifelse( 
      sum(ls.T.CH[[ite]][i, c(4:6)])>0,
      paste(0, sum(ls.T.CH[[ite]][i, c(4:6)]), sep = ""),
      ifelse(CH[i,1]!="..",
             ifelse(sum(ls.T.CH[[ite]][i, c(7:9)])>0,
                    paste("+", 0, sep=""),
                    paste("-", 0, sep="")),
             ".."))
    ...
    CH[i,3] <- ifelse(
      sum(ls.T.CH[[ite]][i, c(7:9)])>0,
      paste(0, sum(ls.T.CH[[ite]][i, c(7:9)]), sep = ""),
      ifelse(CH[i,2]!="..",
             ifelse(sum(ls.T.CH[[ite]][i, c(7:9)])>0,
                    paste("+", 0, sep=""),
                    paste("-", 0, sep="")),
  ...
  for (j in 1:length(ls.y)){
    ls.y[j] <- ls.y[j]-sum(ls.T.CH[[ite]][,j])}
  ...
  km <- numeric()
  km <- sum(ls.T.CH[[ite]][,1]==1|ls.T.CH[[ite]][,2]==1|ls.T.CH[[ite]][,3]==1)
  ...
  Mark.list.Tc[[ite]]  <- assign(paste("Mark", ite, sep = "_"), mod)
  Mark.list.summary.Tc[[ite]]  <- assign(paste("Mark", ite, sep = "_"), summary(mod))
  ...
  T.output.ZPNEc[1,ite] <- mean(N)
  T.output.ZPNEc[2,ite] <- mean(N.se)
  T.output.ZPNEc[7,ite] <- phi
  T.output.ZPNEc[8,ite] <- phi.se
  T.output.ZPNEc[9,ite] <- mean(p)
  T.output.ZPNEc[10,ite] <- mean(p.se)
  ...
  if(ite==iter){
    ...
    save(T.output.ZPNEc,file="T-outputZPNEc.RData")
    setwd("C:/Users/Tati/Dropbox/PhD/Thesis/!! WHOLE DISSERTATION !!/FINAL ANALYSIS/Data/ZPNEc")
    save(Mark.list.Tc,file="T-Mark-list-ZPNEc.RData")
    save(TN.ZPNEc,file="TN-ZPNEc.RData")
    save(TNs.ZPNEc,file="TNs-ZPNEc.RData")
    save(Mark.list.summary.Tc,file="T-Mark-list-summ-ZPNEc.RData")}
...
for(ite in 1:iter)
{
  ...
  for(i in 1:nrow(ls.T.CH2[[ite]])) # Loop through individuals so only the ones marked in marking occasions (first day of each field trip) can actually be captured by observation
  {
    ls.T.CH2[[ite]][i,2] <- ifelse(ls.T.CH[[ite]][i,1]==0,0,ls.T.CH[[ite]][i,2])
    ls.T.CH2[[ite]][i,3] <- ifelse(ls.T.CH[[ite]][i,1]==0,0,ls.T.CH[[ite]][i,3])
    ls.T.CH2[[ite]][i,5] <- ifelse(sum(ls.T.CH[[ite]][i,c(1,4)])==0,0,ls.T.CH[[ite]][i,5])
    ls.T.CH2[[ite]][i,6] <- ifelse(sum(ls.T.CH[[ite]][i,c(1,4)])==0,0,ls.T.CH[[ite]][i,6])
    ls.T.CH2[[ite]][i,8] <- ifelse(sum(ls.T.CH[[ite]][i,c(1,4,7)])==0,0,ls.T.CH[[ite]][i,8])
    ls.T.CH2[[ite]][i,9] <- ifelse(sum(ls.T.CH[[ite]][i,c(1,4,7)])==0,0,ls.T.CH[[ite]][i,9])
  }
  
  ls.CH2.y <- ls.T.CH2[[ite]]
  
  ls.T.CH2[[ite]] <- ls.T.CH2[[ite]][,-c(1,4,7)] # Remove marking occasions
  ls.T.CH2[[ite]] <- ls.T.CH2[[ite]][apply(ls.T.CH2[[ite]][,-1], 1, function(x) !all(x==0)),] # Delete individuals with no captures
  ...
  ifelse(is.null(nrow(ls.T.CH2[[ite]])),next,{ # CREATE A LOOP TO ONLY ORGANIZE DATA IF nrow(CH) IS NOT NULL
    
    for (i in 1:nrow(ls.T.CH2[[ite]])) {
      
      # FOR THE FIRST FIELD TRIP
      CH2[i,1] <- ifelse( 
        sum(ls.T.CH2[[ite]][i, c(1:2)])>0,
        paste(0, sum(ls.T.CH2[[ite]][i, c(1:2)]), sep = ""),
        "..")
      
      # FOR THE SECOND FIELD TRIP
      CH2[i,2] <- ifelse(
        sum(ls.T.CH2[[ite]][i, c(3:4)])>0,
        paste(0, sum(ls.T.CH2[[ite]][i, c(3:4)]), sep = ""),
        ifelse(CH2[i,1]!="..",
               ifelse(sum(ls.T.CH2[[ite]][i, c(5:6)])>0,
                      paste("+", 0, sep=""),
                      paste("-", 0, sep="")),
               ".."))
      ...
      # FOR THE THIRD FIELD TRIP
      CH2[i,3] <- ifelse(
        sum(ls.T.CH2[[ite]][i, c(5:6)])>0,
        paste(0, sum(ls.T.CH2[[ite]][i, c(5:6)]), sep = ""),
        ifelse(CH2[i,2]!="..",
               ifelse(sum(ls.T.CH2[[ite]][i, c(5:6)])>0,
                      paste("+", 0, sep=""),
                      paste("-", 0, sep="")),
    ...
    # FOR EACH Known Marks
    km2 <- numeric()
    km2 <- colSums(ls.T.CH2[[ite]])[1]+colSums(ls.T.CH2[[ite]])[2]-sum(ls.T.CH2[[ite]][,1]==1&ls.T.CH2[[ite]][,2]==1)
    ...
    Mark2.list.Tc[[ite]]  <- assign(paste("Mark", ite, sep = "_"), mod)
    Mark2.list.summary.Tc[[ite]]  <- assign(paste("Mark", ite, sep = "_"), summary(mod))
    ...
    T.output.ZPNEs[1,ite] <- mean(N)
    T.output.ZPNEs[2,ite] <- mean(N.se)
    T.output.ZPNEs[7,ite] <- phi
    T.output.ZPNEs[8,ite] <- phi.se
    T.output.ZPNEs[9,ite] <- mean(p)
    T.output.ZPNEs[10,ite] <- mean(p.se)
  ...
  if(ite==iter){
    setwd("C:/Users/Tati/Dropbox/PhD/Thesis/!! WHOLE DISSERTATION !!/FINAL ANALYSIS/Data/Outputs Excluded Rhat")
    save(T.output.ZPNEs,file="T-outputZPNEs.RData")
    setwd("C:/Users/Tati/Dropbox/PhD/Thesis/!! WHOLE DISSERTATION !!/FINAL ANALYSIS/Data/ZPNEs")
    save(Mark2.list.Tc,file="T-Mark-list-ZPNEs.RData")
    save(TN.ZPNEs,file="TN-ZPNEs.RData")
    save(TNs.ZPNEs,file="TNs-ZPNEs.RData")
    save(Mark2.list.summary.Tc,file="T-Mark-list-summ-ZPNEs.RData")}
...
save.image(file="C:/Users/Tati/Dropbox/PhD/Thesis/!! WHOLE DISSERTATION !!/FINAL ANALYSIS/Backups/RESULTS-MARK-TRENDS.RData")
